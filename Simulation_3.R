###type 1 error and power : same sparse covariance matrix between compared groups
type1error_equal_sparse<-function(p,n1,n2,alpha,N)
  
{
  ...
  for (j in 1:N) {
    ...
    q=floor(3*sqrt(p))
    b1=runif(ceiling(q*q*0.25),max=-0.5,min = -1)
    b2=runif(ceiling(q*q*0.25),max=1,min = 0.5)
    b3=sample(c(b1,b2,rep(0,ceiling(q*q*0.5))),q*q,replace = TRUE)
    B=matrix(b3,q,q)
    B[upper.tri(B)]<-0
    B=B + t(B) - diag(diag(B))
    e=min(eigen(B)$val)
    epson<-max(-e,0)+0.05
    A1=B+epson*diag(1,q,q)
    A2=diag(1,p-q,p-q)
    library(Matrix)
    omega1 <- as.matrix(bdiag(A1,A2))
    library(mvtnorm)
...
power_equal_sparse<-function(p,n1,n2,alpha,N,ss)
  
{
  ...
  for (j in 1:N) {
    ...
    d=runif(p,min=-sqrt(4),max = sqrt(4))
    s1=floor(ss*p)
    index=sample(seq(1,p),s1)
    d[-index]=0
    delta=d
    
    
    mu2=mu1-delta*sqrt(w)*sqrt((log(p))/n2)
    Z_2=rmvnorm(n=n2,mean = mu2,sigma=omega2)
    W_2=exp(Z_2)
    X_2=matrix(0,n2,p)
    for (i in 1:n2) { 
